...
public class DatasourceMapCache {
  ...
  @Autowired private DatasourceMapMapper datasourceMapMapper;

  private static final long CLEAN_PERIOD = 3 * 60 * 1000;
  ...
  public void init() {
    ...
    Utils.defaultScheduler()
        .scheduleWithFixedDelay(
            new CleanRunnable(), CLEAN_PERIOD, CLEAN_PERIOD, TimeUnit.MILLISECONDS);
  }
  ...
  class CleanRunnable implements Runnable {
    ...
    public void run() {
      ...
      try {
        ...
        logger.warn("DatasourceMapCache clean runner refresh error, {}", t.getMessage(), t);
      }

      Set<String> badInstances = new HashSet<>();
      datasourceMaps.forEach(
          datasourceMap -> {
            String instance = datasourceMap.getInstance();
            if (badInstances.contains(instance)) {
              return;
            }
